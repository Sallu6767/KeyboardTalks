const App = (() => {
    ...
    function setupAutostartNotice() {
        const banner = document.getElementById("autostart-notice");
        const dismissBtn = document.getElementById("btn-dismiss-autostart-notice");

        if (!banner || !dismissBtn) return;

        const hasSeen = AppState.get("has_seen_autostart_notice");
        ...
        dismissBtn.addEventListener("click", async () => {
            try {
                await invoke("dismiss_autostart_notice");
                AppState.set("has_seen_autostart_notice", true);
                banner.classList.add("hidden");
            } catch (error) {
                console.error("[App] Failed to dismiss autostart notice:", error);
            }
        });
    }
