...
pub fn set_soundpack(pack_id: String) -> Result<String, String> {
    ...
    config::update(|c| {
        c.active_soundpack = pack_id.clone();
    });
...
pub fn import_custom_sound(file_path: String) -> Result<String, String> {
    ...
    fs::copy(&source_path, &final_path).map_err(|e| {
        format!("Could not copy sound file: {}", e)
    })?;
...
pub fn set_key_mapping(key_name: String, file_name: String) -> Result<String, String> {
    ...
    if !file_path.exists() {
        return Err(format!(
            "Sound file '{}' not found. Import it first.",
            file_name
        ));
    }

    {
        config::update(|c| {
            c.custom_mappings
                .insert(key_name.clone(), file_name.clone());
        });
    }
