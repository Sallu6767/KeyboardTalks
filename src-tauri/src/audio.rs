...
impl AudioEngine {
    pub fn new(soundpack_name: &str) -> Result<Self, String> {
        ...
        let _ = engine.load_soundpack(soundpack_name);
    ...
    pub fn load_soundpack(&mut self, pack_name: &str) -> Result<(), String> {
        ...
        for category in KEY_CATEGORIES {
            ...
            match self.load_bundled_sound_or_fallback(pack_name, &filename, category) {
                Ok(bytes) => {
                    new_sounds.insert(category.to_string(), Arc::new(bytes));
                }
                Err(_) => {
                    any_fallback = true;
                    new_sounds.insert(category.to_string(), Arc::new(generate_sine_wave_wav(440.0, 0.15, 44100)));
                }
            }
        }
...
fn retry_until_real_sounds_load(pack: String) {
    let mut attempts = 0;
    let max_attempts = 40;
    ...
    loop {
        ...
        attempts += 1;
        println!("[Audio] Background retry #{} for soundpack '{}'", attempts, pack);

        if switch_soundpack(&pack).is_ok() {
            let still_fallback = ENGINE.lock()
                .0.as_ref()
                .map(|e| e.used_fallback)
                .unwrap_or(false);
            if !still_fallback {
                println!("[Audio] Real soundpack loaded after {} retries", attempts);
                return;
            }
        }

        if attempts >= max_attempts {
            eprintln!("[Audio] Giving up after {} attempts, staying on fallback", attempts);
            return;
        }
    }
}
    ...
    thread::spawn(|| {
        ...
        match AudioEngine::new(&cfg.active_soundpack) {
            Ok(engine) => {
                ...
                println!("Audio engine initialized (fallback used: {})", used_fallback);

                if used_fallback {
                    retry_until_real_sounds_load(cfg.active_soundpack.clone());
                }
            }
            Err(e) => eprintln!("Failed to initialize audio engine: {}", e),
        }
    });
}
