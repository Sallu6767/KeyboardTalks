...
const GITHUB_RELEASES_URL =
  "https://github.com/Sallu6767/KeyboardTalks/releases/latest";
  ...
  return (
    <section id="hero" className="relative min-h-screen flex flex-col items-center justify-center px-4 py-20 overflow-hidden">
      ...
      <p className="relative mt-6 text-center text-[#94A3B8] text-lg max-w-xl leading-relaxed">
        ...
        Try in the web first. If you like it, buy the pro pass, download the app, and paste the 
        license key in the app to use system wide and to assign any custom sound to any key.
      </p>

      <div id="download" className="relative mt-10 flex flex-col sm:flex-row gap-3 w-full max-w-md">
      ...
      <div className="relative mt-20 grid grid-cols-2 sm:grid-cols-5 gap-4 w-full max-w-2xl">
        {[
          { icon: "⚡", label: "~3-8ms latency" },
          { icon: "🎵", label: "5 soundpacks" },
          { icon: "🔇", label: "Mute anytime" },
          { icon: "🪟", label: "System-wide" },
          { icon: "🚫", label: "Turn Off Completely" },
        ].map((feature) => (
          <div
            key={feature.label}
            className="flex flex-col items-center gap-2 p-3 bg-[#111D3A] border border-[rgba(255,255,255,0.06)] text-center"
          >
